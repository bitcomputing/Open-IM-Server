...
func PostCtx(ctx context.Context, url string, data interface{}, timeOutSecond int) (content []byte, err error) {
	jsonStr, err := json.Marshal(data)
	if err != nil {
		return nil, err
	}
...
func CallBackPostReturnCtx(ctx context.Context, url, callbackCommand string, input, output interface{}, timeOut int) error {
	v := urlLib.Values{}
	v.Set("callbackCommand", callbackCommand)
	url = url + "?" + v.Encode()
	b, err := PostCtx(ctx, url, input, timeOut)
	if err != nil {
		return err
	}
	if err = json.Unmarshal(b, output); err != nil {
