...
func ExtendMessageUpdatedNotification(ctx context.Context, operationID, sendID string, sourceID string, senderPlatformID, sessionType int32,
	...
	if len(keyMap) == 0 {
		logger.Error("all key set failed can not send notification", *req)
		return
	}
	...
	messageReactionSender(ctx, operationID, sendID, sourceID, senderPlatformID, sessionType, constant.ReactionMessageModifier, utils.StructToJsonString(m), isHistory, isReactionFromCache)
}

func ExtendMessageAddedNotification(ctx context.Context, operationID, sendID string, sourceID string, senderPlatformID, sessionType int32,
...
func ExtendMessageDeleteNotification(ctx context.Context, operationID, sendID string, sourceID string, senderPlatformID, sessionType int32,
	...
	messageReactionSender(ctx, operationID, sendID, sourceID, senderPlatformID, sessionType, constant.ReactionMessageDeleter, utils.StructToJsonString(m), isHistory, isReactionFromCache)
}

func messageReactionSender(ctx context.Context, operationID, sendID string, sourceID string, senderPlatformID, sessionType, contentType int32, content string, isHistory bool, isReactionFromCache bool) {
	logger := logx.WithContext(ctx)
	...
	if err != nil {
		...
		logger.Error("SendMsg rpc failed, ", pbData.String(), reply.ErrCode, reply.ErrMsg)
	}
