...
func (s *friendServer) RegisterLegacyDiscovery() {
...
func (s *friendServer) AddBlacklist(ctx context.Context, req *pbFriend.AddBlacklistReq) (*pbFriend.AddBlacklistResp, error) {
	ctx = logx.ContextWithFields(ctx, logx.Field("op", req.CommID.OperationID))
	logger := logx.WithContext(ctx)
	ok := token_verify.CheckAccess(req.CommID.OpUserID, req.CommID.FromUserID)
	if !ok {
		logger.Error("CheckAccess false: ", req.CommID.OpUserID, req.CommID.FromUserID)
	...
	cacheResp, err := s.cacheClient.DelBlackIDListFromCache(ctx, &pbCache.DelBlackIDListFromCacheReq{UserID: req.CommID.FromUserID, OperationID: req.CommID.OperationID})
	if err != nil {
		logger.Error(err)
...
func (s *friendServer) AddFriend(ctx context.Context, req *pbFriend.AddFriendReq) (*pbFriend.AddFriendResp, error) {
	...
	callbackResp := callbackBeforeAddFriend(ctx, req)
	if callbackResp.ErrCode != 0 {
		logger.Error(callbackResp)
	}
	...
	if err != nil {
		logger.Error(err.Error(), req.CommID.ToUserID)
	...
	if err != nil {
		logger.Error(err.Error(), req.CommID.FromUserID)
		return &pbFriend.AddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: err.Error()}}, nil
	}
	logger.Debug("toUserID", userIDList, "fromUserID", userIDList2)
	...
	if isSend {
		if _, err := imdb.GetUserByUserID(ctx, req.CommID.ToUserID); err != nil {
			logger.Error(err.Error(), req.CommID.ToUserID)
		...
		logger.Debug("UpdateFriendApplication args ", friendRequest)
		//err := imdb.InsertFriendApplication(&friendRequest)
		err := imdb.InsertFriendApplication(ctx, &friendRequest,
		...
		if err != nil {
			logger.Error(err.Error(), friendRequest)
			return &pbFriend.AddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
		}
...
func (s *friendServer) ImportFriend(ctx context.Context, req *pbFriend.ImportFriendReq) (*pbFriend.ImportFriendResp, error) {
	...
	if !utils.IsContain(req.OpUserID, config.Config.Manager.AppManagerUid) {
		logger.Error("not authorized", req.OpUserID, config.Config.Manager.AppManagerUid)
	...
	if _, err := imdb.GetUserByUserID(ctx, req.FromUserID); err != nil {
		logger.Error(err.Error(), req.FromUserID)
	...
	for _, v := range req.FriendUserIDList {
		logger.Debug("FriendUserIDList ", v)
		if _, fErr := imdb.GetUserByUserID(ctx, v); fErr != nil {
			logger.Error(fErr.Error(), v)
			resp.UserIDResultList = append(resp.UserIDResultList, &pbFriend.UserIDResult{UserID: v, Result: -1})
		} else {
			if _, err := imdb.GetFriendRelationshipFromFriend(ctx, req.FromUserID, v); err != nil {
				//Establish two single friendship
				toInsertFollow := db.Friend{OwnerUserID: req.FromUserID, FriendUserID: v}
				err1 := imdb.InsertToFriend(ctx, &toInsertFollow)
				if err1 != nil {
					logger.Error(err1.Error(), toInsertFollow)
					resp.UserIDResultList = append(resp.UserIDResultList, &pbFriend.UserIDResult{UserID: v, Result: -1})
					continue
				}
				toInsertFollow = db.Friend{OwnerUserID: v, FriendUserID: req.FromUserID}
				err2 := imdb.InsertToFriend(ctx, &toInsertFollow)
				if err2 != nil {
					logger.Error(err2.Error(), toInsertFollow)
					resp.UserIDResultList = append(resp.UserIDResultList, &pbFriend.UserIDResult{UserID: v, Result: -1})
					continue
				}
				resp.UserIDResultList = append(resp.UserIDResultList, &pbFriend.UserIDResult{UserID: v, Result: 0})
				logger.Debug(resp.UserIDResultList)
				chat.FriendAddedNotification(ctx, req.OperationID, req.OpUserID, req.FromUserID, v)
			} else {
				resp.UserIDResultList = append(resp.UserIDResultList, &pbFriend.UserIDResult{UserID: v, Result: 0})
			}
		}
	...
	cacheResp, err := s.cacheClient.DelFriendIDListFromCache(ctx, &pbCache.DelFriendIDListFromCacheReq{UserID: req.FromUserID, OperationID: req.OperationID})
	if err != nil {
	...
	for _, userID := range req.FriendUserIDList {
		cacheResp, err := s.cacheClient.DelFriendIDListFromCache(ctx, &pbCache.DelFriendIDListFromCacheReq{UserID: userID, OperationID: req.OperationID})
		if err != nil {
			logger.Error(err.Error())
		}
		if cacheResp != nil && cacheResp.CommonResp != nil {
			if cacheResp.CommonResp.ErrCode != 0 {
				logger.Error(req.OperationID, "DelBlackIDListFromCache rpc logic call failed ", cacheResp.String())
				resp.CommonResp.ErrCode = 500
				resp.CommonResp.ErrMsg = cacheResp.CommonResp.ErrMsg
				return &resp, nil
			}
...
func (s *friendServer) AddFriendResponse(ctx context.Context, req *pbFriend.AddFriendResponseReq) (*pbFriend.AddFriendResponseResp, error) {
	...
	friendRequest, err := imdb.GetFriendApplicationByBothUserID(ctx, req.CommID.ToUserID, req.CommID.FromUserID)
	if err != nil {
		logger.Error(err.Error(), req.CommID.ToUserID, req.CommID.FromUserID)
	...
	err = imdb.UpdateFriendApplication(ctx, friendRequest)
	if err != nil {
		logger.Error(err.Error(), friendRequest)
		return &pbFriend.AddFriendResponseResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}
	...
	if req.HandleResult == constant.FriendFlag {
		...
		_, err := imdb.GetFriendRelationshipFromFriend(ctx, req.CommID.FromUserID, req.CommID.ToUserID)
		if err == nil {
			logger.Info(req.CommID.FromUserID, req.CommID.ToUserID)
		} else {
			//Establish two single friendship
			...
			err = imdb.InsertToFriend(ctx, &toInsertFollow)
			if err != nil {
				logger.Error(err.Error(), toInsertFollow)
				return &pbFriend.AddFriendResponseResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
			}
			isInsert = 1
		}

		_, err = imdb.GetFriendRelationshipFromFriend(ctx, req.CommID.ToUserID, req.CommID.FromUserID)
		if err == nil {
			logger.Info(req.CommID.ToUserID, req.CommID.FromUserID)
		} else {
		...
		if isInsert == 1 {
			...
			respPb, err := s.cacheClient.DelFriendIDListFromCache(ctx, delFriendIDListFromCacheReq)
			if err != nil {
				logger.Error(err.Error())
			...
			if respPb.CommonResp.ErrCode != 0 {
				logger.Error(respPb.CommonResp.String())
			...
			chat.FriendAddedNotification(ctx, req.CommID.OperationID, req.CommID.OpUserID, req.CommID.FromUserID, req.CommID.ToUserID)
		}
	}
...
func (s *friendServer) DeleteFriend(ctx context.Context, req *pbFriend.DeleteFriendReq) (*pbFriend.DeleteFriendResp, error) {
	...
	err := imdb.DeleteSingleFriendInfo(ctx, req.CommID.FromUserID, req.CommID.ToUserID)
	if err != nil {
		logger.Error(err.Error(), req.CommID.FromUserID, req.CommID.ToUserID)
		return &pbFriend.DeleteFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrAccess.ErrCode, ErrMsg: constant.ErrAccess.ErrMsg}}, nil
	}

	respPb, err := s.cacheClient.DelFriendIDListFromCache(ctx, &pbCache.DelFriendIDListFromCacheReq{OperationID: req.CommID.OperationID, UserID: req.CommID.FromUserID})
	if err != nil {
		logger.Error(err.Error())
...
func (s *friendServer) SetFriendRemark(ctx context.Context, req *pbFriend.SetFriendRemarkReq) (*pbFriend.SetFriendRemarkResp, error) {
	...
	err := imdb.UpdateFriendComment(ctx, req.CommID.FromUserID, req.CommID.ToUserID, req.Remark)
	if err != nil {
		logger.Error(req.CommID.FromUserID, req.CommID.ToUserID, req.Remark, err.Error())
	...
	if err != nil {
		logger.Error(req.CommID.FromUserID, err.Error())
		return &pbFriend.SetFriendRemarkResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}
	chat.FriendRemarkSetNotification(ctx, req.CommID.OperationID, req.CommID.OpUserID, req.CommID.FromUserID, req.CommID.ToUserID)
...
func (s *friendServer) RemoveBlacklist(ctx context.Context, req *pbFriend.RemoveBlacklistReq) (*pbFriend.RemoveBlacklistResp, error) {
	...
	err := imdb.RemoveBlackList(ctx, req.CommID.FromUserID, req.CommID.ToUserID)
	if err != nil {
		logger.Error(err.Error(), req.CommID.FromUserID, req.CommID.ToUserID)
		return &pbFriend.RemoveBlacklistResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrAccess.ErrCode, ErrMsg: constant.ErrAccess.ErrMsg}}, nil
	...
	if cacheResp.CommonResp.ErrCode != 0 {
		logger.Error(cacheResp.CommonResp.String())
...
func (s *friendServer) GetFriendList(ctx context.Context, req *pbFriend.GetFriendListReq) (*pbFriend.GetFriendListResp, error) {
	...
	for _, friendUser := range friendList {
		...
		cp.FriendDBCopyOpenIM(ctx, &friendUserInfo, friendUser)
		logger.Debug(req.CommID.OperationID, "friends : ", friendUser, "openim friends: ", friendUserInfo)
...
func (s *friendServer) GetFriendApplyList(ctx context.Context, req *pbFriend.GetFriendApplyListReq) (*pbFriend.GetFriendApplyListResp, error) {
	...
	ApplyUsersInfo, err := imdb.GetReceivedFriendsApplicationListByUserID(ctx, req.CommID.FromUserID)
	if err != nil {
		logger.Error(err.Error(), req.CommID.FromUserID)
	...
	for _, applyUserInfo := range ApplyUsersInfo {
		...
		cp.FriendRequestDBCopyOpenIM(ctx, &userInfo, &applyUserInfo)
...
func (s *friendServer) GetSelfApplyList(ctx context.Context, req *pbFriend.GetSelfApplyListReq) (*pbFriend.GetSelfApplyListResp, error) {
	...
	usersInfo, err := imdb.GetSendFriendApplicationListByUserID(ctx, req.CommID.FromUserID)
	if err != nil {
		logger.Error(err.Error(), req.CommID.FromUserID)
	...
	for _, selfApplyOtherUserInfo := range usersInfo {
		...
		cp.FriendRequestDBCopyOpenIM(ctx, &userInfo, &selfApplyOtherUserInfo)
		//u, err := imdb.GetUserByUserID(userInfo.FromUserID)
		//if err != nil {
		//	log.Error(req.CommID.OperationID, "GetUserByUserID", userInfo.FromUserID)
		//	continue
