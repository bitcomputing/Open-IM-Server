...
func (mc *OnlineHistoryMongoConsumerHandler) handleChatWs2Mongo(cMsg *sarama.ConsumerMessage, msgKey string, _ sarama.ConsumerGroupSession) {
	...
	logx.Info(msgFromMQ.TriggerID, "BatchInsertChat2DB userID: ", msgFromMQ.AggregationID, "msgFromMQ.LastSeq: ", msgFromMQ.LastSeq)
	err = db.DB.BatchInsertChat2DB(msgFromMQ.AggregationID, msgFromMQ.MessageList, msgFromMQ.TriggerID, msgFromMQ.LastSeq)
	if err != nil {
		logx.Error(msgFromMQ.TriggerID, "single data insert to mongo err", err, msgFromMQ.MessageList, msgFromMQ.AggregationID, msgFromMQ.TriggerID)
	} else {
		err = db.DB.DeleteMessageFromCache(msgFromMQ.MessageList, msgFromMQ.AggregationID, msgFromMQ.GetTriggerID())
		if err != nil {
			logx.Error(msgFromMQ.TriggerID, "remove cache msg from redis  err", err, msgFromMQ.MessageList, msgFromMQ.AggregationID, msgFromMQ.TriggerID)
		}
	}
	for _, v := range msgFromMQ.MessageList {
		if v.MsgData.ContentType == constant.DeleteMessageNotification {
			...
			if err != nil {
				logx.Error(msgFromMQ.TriggerID, "tips unmarshal err:", err.Error(), v.String())
				continue
			}
			err = proto.Unmarshal(tips.Detail, &DeleteMessageTips)
			if err != nil {
				logx.Error(msgFromMQ.TriggerID, "deleteMessageTips unmarshal err:", err, v.String())
				continue
			}
			if unexistSeqList, err := db.DB.DelMsgBySeqList(DeleteMessageTips.UserID, DeleteMessageTips.SeqList, v.OperationID); err != nil {
				logx.Error(v.OperationID, utils.GetSelfFuncName(), "DelMsgBySeqList args: ", DeleteMessageTips.UserID, DeleteMessageTips.SeqList, v.OperationID, err.Error(), unexistSeqList)
			}
		}
	}
}
...
func (och *OnlineHistoryMongoConsumerHandler) ConsumeClaim(sess sarama.ConsumerGroupSession,
	...
	for msg := range claim.Messages() {
		logx.Debug("kafka get info to mongo", "msgTopic", msg.Topic, "msgPartition", msg.Partition, "msg", string(msg.Value), "key", string(msg.Key))
		if len(msg.Value) != 0 {
			och.msgHandle[msg.Topic](msg, string(msg.Key), sess)
		} else {
			logx.Error("mongo msg get from kafka but is nil", msg.Key)
		}
