...
func (pc *PersistentConsumerHandler) handleChatWs2Mysql(cMsg *sarama.ConsumerMessage, msgKey string, _ sarama.ConsumerGroupSession) {
	msg := cMsg.Value
	logx.Info("msg come here mysql!!!", "msg", string(msg), msgKey)
	var tag bool
	msgFromMQ := pbMsg.MsgDataToMQ{}
	err := proto.Unmarshal(msg, &msgFromMQ)
	if err != nil {
		logx.Error(msgFromMQ.OperationID, "msg_transfer Unmarshal msg err", "msg", string(msg), "err", err.Error())
		return
	}

	logger := logx.WithContext(context.Background()).WithFields(logx.Field("op", msgFromMQ.OperationID))
	logger.Debug("proto.Unmarshal MsgDataToMQ", msgFromMQ.String())
	...
	if isPersist {
		...
		if tag {
			logger.Info("msg_transfer msg persisting", string(msg))
			if err = im_mysql_msg_model.InsertMessageToChatLog(&msgFromMQ); err != nil {
				logger.Error("Message insert failed", "err", err.Error(), "msg", msgFromMQ.String())
				return
			}
		}
...
func (pc *PersistentConsumerHandler) ConsumeClaim(sess sarama.ConsumerGroupSession,
	...
	for msg := range claim.Messages() {
		logx.Debug("", "kafka get info to mysql", "msgTopic", msg.Topic, "msgPartition", msg.Partition, "msg", string(msg.Value), "key", string(msg.Key))
		if len(msg.Value) != 0 {
