...
func DelMsg(c *gin.Context) {
	...
	respPb, err := msgClient.DelMsgList(c.Request.Context(), &reqPb)
	if err != nil {
...
func DelSuperGroupMsg(c *gin.Context) {
	...
	var ok bool
	var errInfo string
	ok, rpcReq.OpUserID, errInfo = token_verify.GetUserIDFromToken(c.Request.Context(), c.Request.Header.Get("token"), req.OperationID)
	if !ok {
	...
	if req.IsAllDelete {
		RpcResp, err := msgClient.DelSuperGroupMsg(c.Request.Context(), rpcReq)
		if err != nil {
...
func ClearMsg(c *gin.Context) {
	...
	RpcResp, err := msgClient.ClearMsg(c.Request.Context(), req)
	if err != nil {
...
func SetMsgMinSeq(c *gin.Context) {
	...
	RpcResp, err := msgClient.SetMsgMinSeq(c.Request.Context(), req)
	if err != nil {
