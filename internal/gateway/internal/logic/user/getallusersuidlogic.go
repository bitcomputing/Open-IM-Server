...
type GetAllUsersUidLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetAllUsersUidLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetAllUsersUidLogic {
	return &GetAllUsersUidLogic{
...
func (l *GetAllUsersUidLogic) GetAllUsersUid(req *types.GetAllUsersUidRequest) (resp *types.GetAllUsersUidResponse, err error) {
	...
	if err != nil {
		logger.Error(rpcReq.OperationID, "call GetAllUsersUid users rpc server failed", err.Error())
		return nil, errors.InternalError.WriteMessage(err.Error())
	}

	return &types.GetAllUsersUidResponse{
